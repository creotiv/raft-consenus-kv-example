...
class Replication(ReplicationInterface):
    ...
    async def replicate(self, new_entries: list[LogEntry], heartbeat: bool = False):
        ...
        await asyncio.gather(
            *(
                self.replicate_peer(cid, c, heartbeat)
    ...
    async def replication_worker(self, follower_id):
        while not self.shutdown.is_set():
            ...
            while True:
                ...
                await self.replicate_peer(follower_id, self.node.clients[follower_id])
