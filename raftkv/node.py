...
class RaftController(RaftNodeInterface):
    ...
    async def append_loop(self):
        while True:
            ...
            if self.state.role != "leader" or self.state.currentTerm != batch[0][0]:
            ...
            if self.state.role != "leader" or self.state.currentTerm != entries[0].term:
    ...
    async def replicate_peer(
        self, client_id: str, client: RaftRPCClient, heartbeat: bool = False
    ) -> bool:
        return await self.replication.replicate_peer(client_id, client, heartbeat)
